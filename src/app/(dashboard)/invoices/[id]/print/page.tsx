...
export default async function InvoicePrintPage(props: {
  ...
  return (
    <PrintPage
      ...
      //
      // Owner UAT (2026-08-31) — เทียบรูปถ่ายกับโปรแกรมเดิม (ACC-Billing) บนเครื่องพิมพ์
      ...
      // ชั่วคราวเป็นการทดลอง — **รอผลพิมพ์กระดาษจริงยืนยันอีกรอบว่าคมชัดขึ้นจริงไหม** ถ้า
      // ไม่ต่างจะลองแนวทางอื่นต่อ (เช่น เพิ่มน้ำหนัก Font/ปรับ DPI ของ Printer Driver เอง)
    ...
    >
      ...
      <InvoicePrintBody
        ...
        disclaimer={
          ...
          // เพื่อให้โตขึ้นตามการขยาย 30% ข้างบนไปด้วยโดยอัตโนมัติ — Owner UAT (2026-08-31)
          // สี text-gray-600 เดิมจางเกินไปเทียบกับส่วนอื่นที่เป็นตัวหนา/สีเข้ม เปลี่ยนเป็น
          // gray-900 ให้เข้มเท่ากัน
          <div className="text-[length:calc(var(--print-body-size)*0.83)] text-gray-900 mb-2">
            <div>
        ...
        pagination={{
          // Owner UAT (2026-08-31) — ต้องส่งตัวคูณ 1.3 เข้าไปด้วย ไม่งั้นระบบจะประมาณ
          // ความจุแถวต่อหน้าจากฟอนต์ปกติ 12px (ไม่รู้เรื่องการขยาย 30% ที่ทำผ่าน CSS
          // ล้วนๆ ข้างบน) ทำให้ใบยาวหลายหน้าตัดหน้าไม่พอดีจริง
          pages: paginateRows(invoice.items, capacityForDocument(template, "INVOICE", 1.3)).map((pageItems) => ({
            items: pageItems,
          ...
          // Owner UAT (2026-08-31) — Signature Block เดิมตรึงขนาดตัวอักษรคงที่ไม่โต
          // ตามเอกสาร ทำให้ดูเล็กไม่สมมาตรกับส่วนอื่นที่ขยายไปแล้ว — ขยายสัดส่วนเดียวกัน
          signature: <PrintSignatureBlock footerNote={template.footerNote} fontScale={1.3} />,
        }}
      />
    </PrintPage>
  );
