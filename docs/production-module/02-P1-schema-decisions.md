...
**Production ไม่เข้าเงื่อนไขเดียวกัน** — กฎข้อ 6 (แก้หลังพิมพ์ต้องสอบย้อนได้ว่าใบเก่าคืออะไร — หมายเหตุ 2026-08-29: Owner ยกเลิกส่วน "ต้องพิมพ์ใหม่ครบทุกชุด" แล้ว การพิมพ์ Revision ใหม่เป็น optional แต่ digital history ต้องครบเหมือนเดิมทุกประการ) + สเปกผ้า/ชั้นโครงสร้างผูกกับของที่ผลิตจริงจับต้องได้ ถ้าผลิตผิดต้องสอบย้อนได้ครบทุกช่องตามวันที่ออกจริง ไม่ใช่สรุปย่อ
...
- **Master Spec batch (2026-08-29)**: `ProductionMasterSpec`+`ProductionMasterFabric`+`ProductionMasterLayer` = สูตรต้นแบบต่อรุ่น นำเข้า bulk จากเอกสาร Owner (source: `docs/production-module/master-spec-data/master-spec-final.json` — corrections 8 ข้อ apply แล้ว) — identity `(specName, variant, thickness, gussetCount)` non-null ทั้ง 4 (sentinel ""/0 — nullable ใน unique key ใช้ไม่ได้เพราะ Postgres ถือ NULL ไม่เท่ากัน), `headKind/headId` = canonical link ไป Product/ProductModel ผูกภายหลังได้ (null = unlinked ติด badge) **ห้ามสร้าง/แก้ Product Master เพื่อให้ mapping ผ่าน** — Master Spec เป็นต้นแบบ prefill เท่านั้น ProductionItem ที่ Confirm แล้วเป็น snapshot อิสระ ไม่ reference กลับ
- **printVisible (2026-08-29)**: `Boolean @default(true)` บน fabric/layer ทั้งฝั่ง Master และ ProductionItem — คนละแกนกับ `displayOverride` (override เปลี่ยนข้อความแต่ยังโชว์แถว / printVisible ซ่อนแถวจากใบพิมพ์) — **ไม่เข้า specHash** เพราะเป็นเรื่องการแสดงผล แต่ canonical data ของแถวที่ซ่อนยังอยู่ครบและยังเข้า hash ตามเนื้อหาจริงเสมอ (ตัวอย่าง use case: ใยเฟลวบางชั้นอยู่ในสูตรจริงแต่ไม่ต้องพิมพ์เพราะพนักงานรู้อยู่แล้ว)
- **Role visibility — documented future security work (ยังไม่ implement, Owner เลือก Option 1 เป็น direction 2026-08-29)**: ข้อมูลลับบางส่วนต้องซ่อนจากบาง role แม้ใน UI (คนละเรื่องกับ printVisible ที่เหมือนกันทุกคน) — แนวทาง: `isRestricted Boolean` ต่อแถว fabric/layer + permission ใหม่ (เช่น `productionMasterSpec.viewRestricted`) ใน `permissions.ts` เดิม — **ต้อง enforce ฝั่ง server** (กรอง/redact ก่อน serialize ออกจาก server action/page ไม่ใช่ซ่อนที่ client) — ยังไม่เพิ่ม field/permission ใดๆ จนกว่า Owner อนุมัติ final enforcement design
